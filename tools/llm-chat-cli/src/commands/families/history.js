...
function createRootCommandSpec() {
  return {
    match: (line) => line === "/",
    parse: () => ({}),
...
function createConvCommandSpec(deps) {
  return createCommandSpec("/conv", async ({ mode, state }) => {
...
function createBackCommandSpec(deps) {
  return createCommandSpec("/back", async ({ mode, handlers, state }) => {
...
function createHistoryOverviewCommandSpec(deps) {
  return createCommandSpec("/history", async ({ mode, state }) => {
...
function createHistoryDateCommandSpec(deps) {
...
function createHistorySearchCommandSpec(deps) {
  return createCommandSpec(
    "/history search",
...
function createHistorySwitchCommandSpec(deps) {
  const { DEFAULT_GATEWAY_MODEL } = deps;
...
function createHistoryAddContextCommandSpec(deps) {
  const { DEFAULT_RAG_MAX_CHARS } = deps;
...
function createHistorySummaryCommandSpec(deps) {
  const { HISTORY_FLOW_SUMMARY, DEFAULT_GATEWAY_MODEL } = deps;
...
function createHistoryPreviewCommandSpec(deps) {
  const { HISTORY_FLOW_PREVIEW } = deps;
...
export function createHistoryCommandSpecs(deps) {
  return [
    createRootCommandSpec(deps),
    createConvCommandSpec(deps),
    createBackCommandSpec(deps),
    createHistoryOverviewCommandSpec(deps),
    createHistoryDateCommandSpec(deps),
    createHistorySearchCommandSpec(deps),
    createHistorySwitchCommandSpec(deps),
    createHistoryAddContextCommandSpec(deps),
    createHistorySummaryCommandSpec(deps),
    createHistoryPreviewCommandSpec(deps)
  ];
}
