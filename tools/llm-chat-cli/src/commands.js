    ...
    createCommandSpec(
      ...
      async ({ mode, handlers, state, parsed, onSaveAndSync, transcriptReader }) => {
        ...
        state.activeTranscript = {
          ...
          routing: deps.captureRoutingFromTranscriptMetadata(metadata)
        };
...
export async function executeChatCommand(
  ...
  const tuiHandlers = deps.createTuiCommandHandlers(handlers);
  const typedLine = String(line || "").trim();
  const alias = deps.parseHistoryConversationAlias(typedLine);
  if (alias && !deps.isHistoryConversationsScreen(state)) {
    writeMessage(mode, tuiHandlers, deps.createHistoryConversationsOnlyMessage(alias.alias));
    return { handled: true, exit: false };
  }
  const normalizedLine = deps.normalizeHistoryInputShortcut(typedLine, state);
  ...
  if (!matched) {
    ...
    if (normalizedLine.startsWith("/")) {
      writeMessage(mode, tuiHandlers, `[chat] unknown command: ${normalizedLine}. Type / to discover commands.`);
      return { handled: true, exit: false };
    }
