...
import { summarizeGDriveImportStatus } from "../domain/gdrive-import.js";
import { asPid } from "../domain/pids.js";
import { DEFAULT_CONFIG_PATH } from "../infrastructure/config-constants.js";
...
async function run() {
  ...
  let service;
  try {
    service = startService(CONFIG_PATH);
  } catch (error) {
    const current = readState();
    writeState({
      ...current,
