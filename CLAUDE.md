...
- `env.js` — the only `.env` parser in the repo. `loadLocalEnv()` **never overwrites an already-set `process.env` key** unless `override: true`, and `MASSA_VAULT_ENV_FILE=off` (or `""`) disables the `.env` layer entirely, mirroring `MASSA_VAULT_HOME_CONFIG=off`. First writer wins for the whole process.
