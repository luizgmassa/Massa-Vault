...
|---|---|---|---|
| `rename-massa-ai-vault` | In progress | T1–T4 landed on `refactor/rename-massa-ai-vault` | Independent validation, then PR |
...
| D16 | Project identity is `massa-ai-vault`: package `massa-ai-vault-tools`, bins `mav`/`mavs`, env prefix `MASSA_AI_VAULT_` (hard cut), MCP server `massa-ai-vault-grounded-sources`; historical text keeps the old name | Aligns with the renamed GitHub repo; future features must not reintroduce `massa-vault` names on live surfaces |
